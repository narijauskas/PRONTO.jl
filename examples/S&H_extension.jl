...
@kwdef struct SplitHold <: Model{29,1}
...
@define_f SplitHold begin
    ...
    H22 = kron(I(2),H2)
    return [mprod(-im*ω*(H00 + sin(x[29])*H11 + (1-cos(x[29]))*H22) )*x[1:28];u[1]]
end
...
@define_m SplitHold begin
    ...
    P = I(28)
    return 1/2 * (x[1:28]-xf)'*P*(x[1:28]-xf)
end


@define_Q SplitHold I(29)
...
PRONTO.Pf(θ::SplitHold, αf, μf, tf) = SMatrix{29,29,Float64}(I(29))

## ----------------------------------- solve the problem ----------------------------------- ##

ψ3 = x_eig(4)[1:7]
ψ4 = x_eig(5)[1:7]
x0 = SVector{29}(vec([ψ3;ψ4;0*ψ3;0*ψ4;0]))
t0,tf = τ = (0,2.8)
